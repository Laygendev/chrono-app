      ...
      <div className="w-full h-full relative">
        <div className="absolute top-1/3 w-full flex flex-col items-center z-10">
          ...
          <AnimatePresence mode="wait">
            {subText && (
              <motion.div
                key={subText}
                className="text-white text-sm font-normal mb-2 text-center"
        ...
        {[...Array(4)].map((_, i) => (
          <motion.div
            ...
            transition={{ duration: 4, repeat: Infinity, repeatDelay: 0.5, delay: i * 1.2, ease: 'easeOut' }}
            style={{ top: '50%', left: '50%', width: '400px', height: '400px', transform: 'translate(-50%, -50%)' }}
          />
        ))}
      </div>
    </div>
